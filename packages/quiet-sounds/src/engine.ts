...
export function durationOf(p: Patch) {
  return (
    Math.max(...p.layers.map((l) => (l.offset || 0) + l.attack + l.decay)) +
    (p.shimmer
      ? p.shimmer.delay *
        Math.max(
          2,
          Math.ceil(
            Math.log(0.00001) / Math.log(Math.max(0.00001, p.shimmer.feedback)),
          ) + 1,
        )
      : 0) +
    0.015
...
export function renderPatch(p: Patch, sampleRate = 48000): Rendered {
  ...
  p.layers.forEach((l) => {
    const start = Math.round((l.offset || 0) * sampleRate),
      total = Math.ceil((l.attack + l.decay) * sampleRate);
    let phase = 0,
      x1 = 0,
      x2 = 0,
      y1 = 0,
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (l.kind === 'tone') {
        ...
        if (l.glideTo)
          hz *= Math.pow(
            l.glideTo / hz,
            Math.min(1, t / (l.glideTime || l.attack + l.decay)),
          );
        ...
        if (i % 64 === 0) {
          const f = Math.min(
            sampleRate * 0.45,
            (l.filterFrequency || 1800) *
              Math.pow(
                (l.filterTo || l.filterFrequency || 1800) /
                  (l.filterFrequency || 1800),
                t / (l.attack + l.decay),
              ),
          );
          const w = (2 * Math.PI * f) / sampleRate,
            c = Math.cos(w),
            alpha = Math.sin(w) / (2 * (l.filterQ || 0.707)),
            a0 = 1 + alpha;
      ...
      if (l.kind === 'tone') toneEnergy += contribution * contribution;
      else noiseEnergy += contribution * contribution;
      if (start + i < data.length) data[start + i] += contribution;
    }
  });
  if (p.shimmer) {
    const d = Math.max(1, Math.round(p.shimmer.delay * sampleRate)),
      delay = new Float32Array(data.length),
      a = 1 - Math.exp((-2 * Math.PI * p.shimmer.lowpass) / sampleRate);
    let low = 0;
    for (let i = d; i < data.length; i++) {
      low += a * (data[i - d] + delay[i - d] * p.shimmer.feedback - low);
      delay[i] = low;
    }
    for (let i = 0; i < data.length; i++) data[i] += delay[i] * p.shimmer.wet;
  }
  let peak = 0,
    sum = 0;
...
export function waveform(data: Float32Array, count = 100) {
  const bins = Array.from({ length: count }, (_, i) => {
    let max = 0;
    for (
      let j = Math.floor((i * data.length) / count);
      j < Math.floor(((i + 1) * data.length) / count);
      j++
    )
...
export function wavBytes(r: Rendered) {
  const out = new ArrayBuffer(44 + r.data.length * 2),
    v = new DataView(out);
  const str = (p: number, s: string) => {
...
export function validatePatch(v: unknown): v is Patch {
      ...
      (l) =>
        ...
        (l.kind === 'tone'
          ...
          : finite(l.filterFrequency, 20, 18000) &&
            ['lowpass', 'bandpass', 'highpass'].includes(
...
export class SoundPlayer {
  ...
  active = new Set<{ node: AudioBufferSourceNode; gain: GainNode }>();
  volume = 0.35;
  muted = true;
  generation = 0;
  ...
  play(r: Rendered, delay = 0, interrupt = true, voiceGain = 1) {
    if (
      !this.context ||
      !this.master ||
      this.muted ||

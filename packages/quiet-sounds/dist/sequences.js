import { CUES, makePatch, timingScale } from './catalog.js';
import { renderPatch } from './engine.js';
...
export const progress = (time, start, duration) => Math.max(0, Math.min(1, (time - start) / duration));
export const smooth = (time, start, duration) => {
    const p = progress(time, start, duration);
    return p * p * (3 - 2 * p);
};
export function buildConversation(actionId, settings, speed = 1, offset = 0, replaceCue) {
    const action = CHAT_ACTIONS.find((a) => a.id === actionId) || CHAT_ACTIONS[0];
    const start = actionId === 'flow' ? 0 : Math.max(0, action.start - 0.12), end = actionId === 'flow' ? FLOW_DURATION : action.end;
    const scale = timingScale(settings) / speed, local = { ...settings, duration: settings.duration / speed };
    const events = actionId === 'flow'
        ? [
            ...CHAT_ACTIONS,
    ...
    const lead = Math.max(0, -offset / 1000), motionDuration = (end - start) * scale + lead;
    const duration = Math.max(motionDuration, ...clips.map((c) => c.at + c.audio.duration)) +
        0.06;
    const data = new Float32Array(Math.ceil(duration * 48000));
    let energy = 0, toneEnergy = 0;
...
export function conversationStage(time) {
    if (time >= 15.33)
        return 'retry';
    if (time >= 13.33)
        return 'failure';
...
export function conversationPose(t, reduced = false) {
    const rise = (start, duration) => reduced ? (t >= start + duration ? 1 : 0) : smooth(t, start, duration);
    const cards = rise(8.32, 0.41), voice = rise(10.18, 0.28), upload = rise(11.22, 0.3), failure = rise(14.17, 0.18), retry = rise(16.07, 0.28);
    const sending = (t >= 1.68 && t < 2.4) ||
        (t >= 5.48 && t < 6.19) ||
        (t >= 6.63 && t < 7.34) ||
        (t >= 13.33 && t < 14.18);
    return {

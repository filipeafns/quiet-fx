...
export function signalLevel(peak: number) {
...
export function heatPosition(level: number, toneShare = 0.5) {
  return Math.pow(clamp01(level), 1.8 - clamp01(toneShare) * 1.2);
}
export function signalRGB(level: number, toneShare = 0.5) {
  const t = heatPosition(level, toneShare) * (stops.length - 1),
    i = Math.min(stops.length - 2, Math.floor(t)),
    f = t - i;
  const pale = 0.34 * (1 - clamp01(level));
  return stops[i].map((v, j) =>
    Math.round((v + (stops[i + 1][j] - v) * f) * (1 - pale) + 255 * pale),
  );
}
export const signalColor = (level: number, toneShare = 0.5) =>
  `rgb(${signalRGB(level, toneShare).join(' ')})`;
export function soundStyle(rendered?: Rendered) {
  const color = signalColor(
    signalLevel(rendered?.peak || 0),
    rendered?.toneShare ?? 0.5,
  );
  return {
    '--sound-color': color,
